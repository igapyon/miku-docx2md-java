...
| `src/ts/docx-package-loader.ts` | `MikuDocx2mdCore.parseDocx` | Loads `word/document.xml`, relationships, styles, numbering, comments, content types, and package files. |
...
| `src/ts/document-comments-parser.ts` | `MikuDocx2mdCore.parseComments` and `jp.igapyon.mikudocx2md.model.ParsedComment` | Preserves referenced Word comments as Markdown footnotes. |
| `src/ts/document-parse-context.ts` | `MikuDocx2mdCore.parseBody` local parse context state | Preserves known anchors and referenced comment tracking. |
| `src/ts/document-block-parser.ts` | `MikuDocx2mdCore.parseBody`, `parseParagraph`, `parseTable` | Preserves block dispatch, summary counters, `sectPr` ignore behavior, and referenced comments. |
| `src/ts/document-inline-parser.ts` | `MikuDocx2mdCore.extractTextRuns`, `renderRun`, `extractTextboxText`, tracked change helpers | Preserves runs, line breaks, hyperlinks, textboxes, Word comment references, tracked insertions / deletions, and unsupported traces. |
...
| `src/ts/markdown-renderer.ts` | `jp.igapyon.mikudocx2md.core.MarkdownRenderer` | Preserves Markdown block rendering, YAML front matter, comment footnotes, and unsupported image placeholders. |
...
| Java-side runtime file API | `jp.igapyon.mikudocx2md.core.MikuDocx2mdFileConverter` and related options/result classes | Java extension that centralizes file output, asset writing, summary writing, and batch directory traversal for CLI and separated adapters. |
