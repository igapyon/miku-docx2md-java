...
| Multiple input files are rejected with the upstream usage error text. | `MikuDocx2mdCliTest.rejectsMultipleInputFiles` | `mvn test -Dtest=MikuDocx2mdCliTest` |
...
The upstream fixture corpus has been copied into `src/test/resources/docx/`.
Expected Markdown captured from the Node CLI is tracked under
`src/test/resources/expected/markdown/`. Expected summary text captured from
the Node runtime is tracked under `src/test/resources/expected/summary/`. Use
`scripts/compare-node-java-cli.sh` for CLI metadata, usage errors, verbose
diagnostics, generated Markdown, summary, stdout Markdown, mixed
